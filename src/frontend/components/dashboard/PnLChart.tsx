...
export default function PnLChart({ data, title = 'Portfolio Performance' }: PnLChartProps) {
  ...
  const CustomTooltip = ({ active, payload }: any) => {
    if (active && payload && payload.length) {
      ...
      return (
        <div className="bg-background-elevated/95 backdrop-blur-sm border border-border/50 rounded-xl p-3 shadow-xl">
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        {/* Timeframe Selector */}
        <div className="flex items-center gap-1 bg-background-elevated rounded-xl p-1 border border-border/50">
          {timeframes.map((tf) => (
            <motion.button
              key={tf.value}
              whileTap={{ scale: 0.95 }}
