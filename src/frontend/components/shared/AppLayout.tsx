...
export default function AppLayout({ children }: AppLayoutProps) {
  ...
  return (
    <div className="app-shell flex">
      {/* Desktop Sidebar */}
      <aside
        className={`hidden lg:flex flex-col fixed left-0 top-0 h-screen bg-background-surface/80 border-r border-border/50 backdrop-blur-xl transition-all duration-300 z-50 ${
          collapsed ? 'w-20' : 'w-64'
        }`}
      >
        {/* Logo */}
        ...
        {/* Bottom nav */}
            ...
            {bottomNavItems.map((item) => {
              ...
              return (
                <Link
                  ...
                  className={`flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all ${
                    isActive
                      ? 'bg-background-elevated/70 text-text-primary'
                      : 'text-text-secondary hover:bg-background-elevated/60 hover:text-text-primary'
                  }`}
                >
          ...
          <button
            onClick={() => signOut()}
            className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-text-secondary hover:bg-danger/10 hover:text-danger transition-all"
          >
        ...
        <button
          ...
          className="absolute -right-3 top-1/2 -translate-y-1/2 w-6 h-6 bg-background-surface/90 border border-border/50 rounded-full flex items-center justify-center text-text-secondary hover:text-text-primary transition-colors"
        >
          {collapsed ? (
      ...
      {/* Mobile Header */}
      <div className="lg:hidden fixed top-0 left-0 right-0 h-16 bg-background-surface/90 border-b border-border/50 backdrop-blur-xl z-50 flex items-center justify-between px-4">
      ...
      <AnimatePresence>
        {mobileMenuOpen && (
          <motion.div
            ...
            className="lg:hidden fixed inset-0 top-16 bg-background-surface/95 z-40 p-4 backdrop-blur-xl"
          >
            <nav className="space-y-1">
              ...
              <div className="pt-4 border-t border-border/40 mt-4">
                ...
                <button
                  ...
                  className="w-full flex items-center gap-3 px-4 py-3 rounded-xl text-text-secondary hover:bg-danger/10 hover:text-danger transition-all"
                >
                  <LogOut className="w-5 h-5" />
