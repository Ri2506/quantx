  ...
  if (!userData) {
    return (
      <div className="text-center py-12">
        ...
        <p className="text-text-primary text-lg">User not found</p>
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1">
          <h1 className="text-2xl font-bold text-text-primary">{user.full_name || 'User'}</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="app-panel p-6">
          <h3 className="text-lg font-semibold text-text-primary mb-4">Basic Info</h3>
        ...
        <div className="app-panel p-6">
          ...
          <div className="space-y-3">
            <div className="flex justify-between">
              ...
              <span className="text-text-primary font-medium">₹{user.capital.toLocaleString()}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-text-secondary">Total Trades</span>
              <span className="text-text-primary font-medium">{user.total_trades}</span>
            </div>
            <div className="flex justify-between">
        ...
        {/* Trading Settings */}
        <div className="app-panel p-6">
          <h3 className="text-lg font-semibold text-text-primary mb-4">Trading Settings</h3>
          <div className="space-y-3 text-sm">
            <div className="flex justify-between">
              <span className="text-text-secondary">Mode</span>
              <span className="text-text-primary capitalize">{user.trading_mode.replace('_', ' ')}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-text-secondary">Risk Profile</span>
              <span className="text-text-primary capitalize">{trading_settings.risk_profile}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-text-secondary">Risk/Trade</span>
              <span className="text-text-primary">{trading_settings.risk_per_trade}%</span>
            </div>
            <div className="flex justify-between">
              <span className="text-text-secondary">F&O Enabled</span>
              <span className={trading_settings.fo_enabled ? 'text-green-500' : 'text-text-muted'}>
      ...
      <div className="app-panel p-6">
        {activeTab === 'overview' && (
          <div>
            <h3 className="text-lg font-semibold text-text-primary mb-4">Active Positions</h3>
            {positions.length > 0 ? (
                ...
                <table className="w-full">
                  ...
                  <tbody className="divide-y divide-border/50">
                    {positions.map((pos: any) => (
                      <tr key={pos.id}>
                        <td className="py-3 text-text-primary font-medium">{pos.symbol}</td>
                        <td className={`py-3 ${pos.direction === 'LONG' ? 'text-green-500' : 'text-red-500'}`}>
                          {pos.direction}
                        </td>
                        <td className="py-3 text-text-secondary">{pos.quantity}</td>
                        <td className="py-3 text-text-secondary">₹{pos.average_price}</td>
                        <td className="py-3 text-text-secondary">₹{pos.current_price}</td>
        ...
        {activeTab === 'trades' && (
          <div>
            ...
            {trades.length > 0 ? (
                  ...
                  <tbody className="divide-y divide-border/50">
                    {trades.map((trade: any) => (
                      <tr key={trade.id}>
                        ...
                        <td className="py-3 text-text-secondary">{trade.quantity}</td>
                        <td className="py-3 text-text-secondary">₹{trade.entry_price}</td>
                        <td className="py-3 text-text-secondary">₹{trade.exit_price}</td>
              ...
              <p className="text-text-muted">No trades found</p>
            )}
          </div>
        )}
            ...
            {payment_history.length > 0 ? (
              <div className="space-y-3">
                {payment_history.map((payment: any) => (
                  <div key={payment.id} className="flex justify-between items-center p-4 bg-background-elevated/70 rounded-lg">
                    <div>
                    ...
                    <div className="text-right">
                      <p className="text-text-primary font-medium">₹{(payment.amount / 100).toLocaleString()}</p>
            ...
            {recent_activity.length > 0 ? (
              <div className="space-y-3">
                {recent_activity.map((activity: any, i: number) => (
                  <div key={i} className="flex items-start gap-4 p-4 bg-background-elevated/70 rounded-lg">
                    <div className="w-10 h-10 bg-background-elevated rounded-full flex items-center justify-center">
                      <History className="w-5 h-5 text-text-secondary" />
                    </div>
                    <div className="flex-1">
                      <p className="text-text-primary">{activity.details}</p>
                      <p className="text-text-muted text-sm capitalize">{activity.action.replace('_', ' ')}</p>
                    </div>
                    <p className="text-text-muted text-sm">
