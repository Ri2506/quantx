...
export default function PnLChart({ data, title = 'Portfolio Performance' }: PnLChartProps) {
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-white/60">Cumulative P&L over time</p>
          </div>
        </div>

        {/* Timeframe Selector */}
        <div className="flex items-center gap-1 bg-background-elevated rounded-xl p-1 border border-d-border">
          {timeframes.map((tf) => (
            <motion.button
              key={tf.value}
              whileTap={{ scale: 0.95 }}
              ...
              className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-all ${
                selectedTimeframe === tf.value
                  ...
                  : 'text-white/60 hover:text-white hover:bg-background-surface'
              }`}
            >
              {tf.label}
            </motion.button>
      ...
      {/* Summary Stats */}
      <div className="grid grid-cols-3 gap-4 mt-6 pt-6 border-t border-d-border">
        <div>
          <p className="text-white/60 text-xs mb-1">Total P&L</p>
          <p className="text-xl font-bold text-up font-mono">
        ...
        <div>
          ...
          <p className="text-xl font-bold text-down font-mono">
